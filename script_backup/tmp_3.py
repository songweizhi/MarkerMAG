    ...
    for each_match in open(file_in_sorted):
        if each_match.startswith('MarkerGene,GenomicSeq,Number'):
            ...
            if linkage_num >= min_linkages:
                if (MarkerGene not in MarkerGene_with_assignment) and (MarkerGene not in MarkerGene_to_ignore):
                    ...
                    if GenomicSeq not in GenomicSeq_best_marker_dict:
                        ...
                        if iden_with_best_marker >= within_genome_16s_divergence_cutoff:
                            ...
                            MarkerGene_to_ignore.add(MarkerGene)
...
blast_results_all_vs_all_16s    = '%s/MBARC26_link_Matam_FakeBins_16S_all_vs_all_blastn.tab' % wd
link_stats_paired               = '%s/MBARC26_link_Matam_FakeBins_stats_paired.txt' % wd
...
filter_linkages_iteratively(link_stats_paired, 'Number', pairwise_16s_iden_dict, within_genome_minimum_iden16s, 2, link_stats_paired_filtered)
