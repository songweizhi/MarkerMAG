...
flk_len                     = 10000
combined_depth_file         = '/Users/songweizhi/Desktop/reference_genomes_combined_best_match_mis0_sorted_depth.txt'
...
gnm_total_16s_depth_dict = {}
gnm_total_16s_len_dict = {}
for each_pos in open(combined_depth_file):
    each_pos_split = each_pos.strip().split('\t')
    ctg_id = each_pos_split[0]
    ctg_pos = int(each_pos_split[1])
    pos_depth = int(each_pos_split[2])
    if ctg_id in s16_ctg_to_gnm_dict:
        if ctg_pos in ctg_to_16s_region_dict[ctg_id]:
            ctg_gnm = s16_ctg_to_gnm_dict[ctg_id]
            if ctg_gnm not in gnm_total_16s_depth_dict:
                gnm_total_16s_depth_dict[ctg_gnm] = 0
            if ctg_gnm not in gnm_total_16s_len_dict:
                gnm_total_16s_len_dict[ctg_gnm] = 0
            gnm_total_16s_depth_dict[ctg_gnm] += pos_depth
            gnm_total_16s_len_dict[ctg_gnm] += 1


print('Genome\tDepth_16S\tDepth_gnm\tCalculated\tReported')
for gnm_id in gnm_total_16s_depth_dict:
    if gnm_id in gnm_to_linked_16s_dict:

        current_gnm_total_16s_depth = gnm_total_16s_depth_dict[gnm_id]
        current_gnm_total_16s_len = gnm_total_16s_len_dict[gnm_id]
        ...
        if current_gnm_mean_16s_depth == 'NA':
            ratio_16s_to_gnm = 'NA'
        else:
            ratio_16s_to_gnm = current_gnm_mean_16s_depth/ref_gnm_depth
            ratio_16s_to_gnm = float("{0:.2f}".format(ratio_16s_to_gnm))
...
for each_gnm in gnm_to_linked_16s_dict:
    ...
    for each_16s_ctg in gnm_16s_ctg_set:
        ...
        for each_16s_region in ctg_16s_region_list:
            ...
            plot_depth_cmd = 'BioSAK plot_sam_depth -r %s.fna -d %s -i %s -s %s -e %s -k 1000 -l %s,%s' % (pwd_ref_gnm, pwd_depth_file, each_16s_ctg, (left_end-flk_len), (right_end+flk_len), left_end, right_end)
