...
'''
reference_genomes_renamed_16S_report_all_sorted	IS>100bp	13296	14.2
reference_genomes_renamed_16S_report_all_sorted	IS<100bp	80321	85.8

reference_genomes_renamed_16S_report_all_sorted	IS>100bp	50056	14.12
reference_genomes_renamed_16S_report_all_sorted	IS<100bp	304358	85.88

reference_genomes_renamed_16S_report_all_sorted	IS>200bp	14114	3.98
reference_genomes_renamed_16S_report_all_sorted	IS<200bp	340300	96.02
'''


sam_basename                        = 'reference_genomes_renamed_16S_report_all_sorted'
...
pwd_sam_file_filtered               = '/Users/songweizhi/Desktop/666/000/%s_all_mis%s_minM%s_IS%s_filtered.sam'         % (sam_basename, mismatch_cutoff, min_M_len_16s, min_insert_size)
if both_pair_mapped is True:
    ...
    pwd_sam_file_filtered           = '/Users/songweizhi/Desktop/666/000/%s_both_mis%s_minM%s_IS%s_filtered.sam'        % (sam_basename, mismatch_cutoff, min_M_len_16s, min_insert_size)
...
large_insert_size_paired_read_pct = large_insert_size_paired_read_num*100/(large_insert_size_paired_read_num + small_insert_size_paired_read_num)
small_insert_size_paired_read_pct = small_insert_size_paired_read_num*100/(large_insert_size_paired_read_num + small_insert_size_paired_read_num)
large_insert_size_paired_read_pct = float("{0:.2f}".format(large_insert_size_paired_read_pct))
small_insert_size_paired_read_pct = float("{0:.2f}".format(small_insert_size_paired_read_pct))
...
ref_gnm_to_read_dict = {}
for each_ref_16s in ref_to_read_dict:
    ref_16s_gnm = each_ref_16s.split('_')[0]
    matched_read_set = ref_to_read_dict[each_ref_16s]
    if ref_16s_gnm not in ref_gnm_to_read_dict:
        ref_gnm_to_read_dict[ref_16s_gnm] = matched_read_set
    else:
        for each_read in matched_read_set:
            ref_gnm_to_read_dict[ref_16s_gnm].add(each_read)


for each_ref_gnm in ref_gnm_to_read_dict:

    ref_gnm_matched_read_set = ref_gnm_to_read_dict[each_ref_gnm]
    matched_read_total_len = 0
    for each_read in ref_gnm_matched_read_set:
        matched_read_total_len += read_len_dict[each_read]

    ref_gnm_16s_gene_set = ref_gnm_to_renamed_16S_id_dict[each_ref_gnm]
    ref_gnm_16s_gene_len_list = [marker_len_dict[s16] for s16 in ref_gnm_16s_gene_set]
    ref_gnm_16s_gene_mean_len = sum(ref_gnm_16s_gene_len_list)/len(ref_gnm_16s_gene_len_list)
    ref_gnm_16s_gene_depth = matched_read_total_len/ref_gnm_16s_gene_mean_len
    ref_gnm_depth = ref_depth_dict[each_ref_gnm]
    ref_gnm_16s_copy_num_calculated = ref_gnm_16s_gene_depth/ref_gnm_depth
    ref_gnm_16s_copy_num_reported = len(ref_gnm_16s_gene_set)
    ref_gnm_16s_copy_num_calculated = float("{0:.2f}".format(ref_gnm_16s_copy_num_calculated))

    ref_gnm_16s_copy_num_normalized = ref_gnm_16s_copy_num_calculated*100/large_insert_size_paired_read_pct
    ref_gnm_16s_copy_num_normalized = float("{0:.2f}".format(ref_gnm_16s_copy_num_normalized))

    print('%s\t%s\t%s' % (each_ref_gnm, ref_gnm_16s_copy_num_normalized, ref_gnm_16s_copy_num_reported))
...
#
#
# print('Genome\tMAG\tREF(calculated)\tREF(reported)')
...
print('%s\tIS>%sbp\t%s\t%s' % (sam_basename, min_insert_size, large_insert_size_paired_read_num, large_insert_size_paired_read_pct))
print('%s\tIS<%sbp\t%s\t%s' % (sam_basename, min_insert_size, small_insert_size_paired_read_num, small_insert_size_paired_read_pct))
