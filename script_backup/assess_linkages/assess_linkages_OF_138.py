...
wd                  = '/Users/songweizhi/Desktop/MarkerMAG_wd/9_Oil_Field/Oil_Field'
bin_id_file         = '%s/bin_id.txt'                                                                   % wd
barrnap_output      = '%s/OF_16S.txt'                                                                   % wd
checkm_output       = '%s/OF_MAG_quality_formatted.txt'                                                 % wd
mean_depth_file     = '%s/OF_42_MAGs_mean_depth.txt'                                                    % wd
seq_file_16s        = '%s/Oil_Field_assembled_16S_uclust_0.999.fasta'                                   % wd

# r95
taxonomy_mag        = '%s/OF_42_MAGs.summary.tsv'                                                       % wd
taxonomy_16s        = '%s/Oil_Field_assembled_16S_uclust_0.999_vs_GTDB_classifications.txt'             % wd
taxonomy_16s_blca   = '%s/Oil_Field_assembled_16S_uclust_0.999_blca_vs_gtdb.txt'                        % wd
gtdb_gnm_to_ssu_txt = '%s/GTDB_ssu_all_r95_gnm_to_ssu.txt'                                              % wd

#r202
taxonomy_mag        = '%s/OF_42_MAGs.summary_r202.tsv'                                                       % wd
taxonomy_16s        = '%s/Oil_Field_assembled_16S_uclust_0.999_vs_GTDB_r202_classifications.txt'             % wd
taxonomy_16s_blca   = '%s/Oil_Field_assembled_16S_uclust_0.999_blca_vs_gtdb_r202.txt'                        % wd
gtdb_gnm_to_ssu_txt = '%s/GTDB_ssu_all_r202_gnm_to_ssu.txt'                                              % wd


taxonomy_16s_silva      = '%s/Oil_Field_assembled_16S_uclust_0.999_vs_SILVA_r138_classifications.txt'      % wd
taxonomy_16s_silva_blca = '%s/Oil_Field_assembled_16S_uclust_0.999_blca_vs_silva_r138.txt'                 % wd


linkage_file        = '%s/OF_0726_45_45_1200_iden99_mismatch2_linkages_by_genome.txt'                                                    % wd

'''
=============================
Linkage accuracy: OF_linkages_by_genome
Rank	All	NA	Correct	Unknown	Total(Accuracy)
phylum	26	0	25	0	25/26(96.15)
class	26	0	24	0	24/26(92.31)
order	26	0	23	0	23/26(88.46)
family	26	0	23	0	23/26(88.46)
genus	26	0	19	2	21/26(80.77)
=============================
Total linkages: 26
Linked MAGs: 19
...
s16_taxon_dict_silva = {}
for each_16s_taxon in open(taxonomy_16s_silva):
    each_16s_taxon_split = each_16s_taxon.strip().split('\t')
    id_16s = each_16s_taxon_split[0]
    taxon_best_hit = each_16s_taxon_split[4].split(' [')[0]
    s16_taxon_dict_silva[id_16s] = taxon_best_hit


def blca_op_to_dict(taxonomy_16s_blca):
    ...
    return s16_taxon_blca_dict_formatted


s16_taxon_blca_dict_formatted = blca_op_to_dict(taxonomy_16s_blca)
s16_taxon_blca_dict_formatted_silva = blca_op_to_dict(taxonomy_16s_silva_blca)
...
for each_linkage in open(linkage_file):
    if each_linkage.startswith('MarkerGene	GenomicSeq'):
        linkage_file_with_assessment_handle.write('%s\tPhylum\tClass\tOrder\tFamily\tGenus\tIdentity\tAln_len\tclosest_ref\tclosest_ref_16s_num\tMAG_taxon\t16S_taxon_GTDB\t16S_taxon_GTDB_BLCA\t16S_taxon_SILVA\t16S_taxon_SILVA_BLCA\n' % each_linkage.strip())
    else:
        ...
        taxon_16s_silva = s16_taxon_dict_silva.get(s16_id, 'NA')
        ...
        taxon_16s_blca_silva = s16_taxon_blca_dict_formatted_silva.get(s16_id, 'NA')
        ...
        line_to_write += '\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' % (iden_16s, aln_len_16s, closest_ref, closest_ref_16s_num, taxon_mag, taxon_16s, taxon_16s_blca, taxon_16s_silva, taxon_16s_blca_silva)
