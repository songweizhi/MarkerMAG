...
if not myfilelist:
    print
    'Please make sure your current directory is YYYYMMDD_MMSS_mer_lt for Evaluate.py.'
    exit()
...
print
'No. of reference genomes: %d' % len(Sentivity.keys())
print
'No. of bins in evaluation: %d' % len(Precision.keys())
print
'No. of sequences assigned reference: %d' % AllCtgSeq
print
'No. of binned sequences: %d' % len(SeqLen.keys())
...
for cluster, refs in Precision.items():
    ...
    fw.write(cluster + ':%f(%d/%d)' % (myp, toprefCount, len(refs)) + '\t' + (
        '\t'.join(['%s:%d' % (x, refs.count(x)) for x in set(refs)])) + '\n')
fw.close()
...
print
'Precision: %f, %f' % (
sum(Precision1_up) / float(sum(Precision1_down)), sum(Precisionw1_up) / float(sum(Precisionw1_down)))
...
for ref, clusters in Sentivity.items():
    ...
    fw.write(ref + ':%f(%d/%d)' % (mys, topClusterCount, len(clusters)) + '\t' + (
        '\t'.join(['%s:%d' % (x, clusters.count(x)) for x in set(clusters)])) + '\n')
fw.close()
...
print
'Sensitivity: %f, %f' % (sum(Sentivity1_up) / float(AllCtgSeq), sum(Sentivityw1_up) / float(AllCtgSeqWeight))
