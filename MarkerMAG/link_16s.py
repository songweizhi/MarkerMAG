...
def link_16s(args, config_dict):
    ...
    combined_linkage_file_tmp                   = '%s/%s_combined_linkages_tmp.txt'                 % (step_2_wd, output_prefix)
    combined_linkage_file_tmp_html              = '%s/%s_combined_linkages_tmp.html'                % (step_2_wd, output_prefix)
    ...
    report_and_log(('Step 2: extracting unmapped reads genome'), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Step 2: extracting unmapped reads 16s'), pwd_log_file, keep_quiet)
    ...
    # extract reads with multiprocessing
    extracted_reads_with_multiprocessing(reads_file_r1, reads_file_r2, extract_list_combined_r1_no_overlap,
                                         extract_list_combined_r2_no_overlap, extracted_reads_folder, num_threads)
    ...
    report_and_log(('Step 2: running SPAdes'), pwd_log_file, keep_quiet)

    spades_log = '%s/%s_SPAdes.log' % (step_2_wd, output_prefix)
    ...
    report_and_log(('Step 2: mapping'), pwd_log_file, keep_quiet)
    ...
    filter_linkages_iteratively(stats_GapFilling_file, 'Number', pairwise_16s_iden_dict, {}, {}, 0, 98, 3, stats_GapFilling_file_filtered)
...
if __name__ == '__main__':
    ...
    link_16s_parser.add_argument('-mp',              required=False, type=str,      default=0,          help='multiple placement during bowtie mapping, default: 0. choose from a,1,2,3...')
    link_16s_parser.add_argument('-cigarM',          required=False, type=int,      default=30,         help='cigarM cutoff, default: 30')
    link_16s_parser.add_argument('-cigarS',          required=False, type=int,      default=30,         help='cigarS cutoff, default: 30')
    link_16s_parser.add_argument('-ri',              required=False, type=float,    default=100,        help='identity cutoff, default: 100')
    link_16s_parser.add_argument('-rc',              required=False, type=float,    default=100,        help='coverage cutoff, default: 100')
